...
class HookAcceptancePayloadTest {
    ...
    fun rejectsUnsupportedSchemaVersion() {
        ...
        assertEquals("schemaVersion must be 3", failure.message)
    }

    @Test
    fun rejectsBlankOrUnsafeSessionIds() {
        val raw = """{"schemaVersion":3,"mode":"always_on","fields":{"tac":4095}}"""
    ...
    fun rejectsNonObjectFieldsAndNonScalarValues() {
        assertThrows(IllegalArgumentException::class.java) {
            HookAcceptancePayload.validate(
                "acceptance-123",
                """{"schemaVersion":3,"acceptanceSessionId":"acceptance-123","mode":"always_on","fields":[]}""",
            )
        }
        assertThrows(IllegalArgumentException::class.java) {
            HookAcceptancePayload.validate(
                "acceptance-123",
                """{"schemaVersion":3,"acceptanceSessionId":"acceptance-123","mode":"always_on","fields":{"operator_name":"HOOK-SESSION:acceptance-123","tac":{"value":4095}}}""",
            )
        }
    }
    ...
    fun rejectsFieldsOutsideTheCellularProfileContract() {
        val failure = assertThrows(IllegalArgumentException::class.java) {
            HookAcceptancePayload.validate(
                "acceptance-123",
                """{"schemaVersion":3,"acceptanceSessionId":"acceptance-123","mode":"always_on","fields":{"operator_name":"HOOK-SESSION:acceptance-123","wifi_ssid":"not-cellular"}}""",
            )
        }
    ...
    fun acceptsCellularControlsAndNeighborJsonAsScalarFields() {
        ...
        assertEquals(
            """{"schemaVersion":3,"acceptanceSessionId":"acceptance-neighbors","mode":"always_on","fields":{"operator_name":"HOOK-SESSION:acceptance-neighbors","signal_fluctuation_enabled":0,"signal_fluctuation_range_db":6,"neighbor_cells_json":"[{\"type\":\"gsm\",\"cid\":2222}]"},"unavailable":[]}""",
            validated.json,
        )
    }
