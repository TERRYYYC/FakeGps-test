...
Code SHA: `144cc659076b8874a33086e17530e22595e08900`
...
- Added Red→Green unit contracts, variant-route instrumentation and a selected-row repository test.
...
both ownership gaps without changing import's safe “never auto-publish” behavior.
...
routing state, not row order. Kept only one compatibility behavior: installations with no recorded
active id publish the legacy oldest row once and atomically record the resolved id.
...
2. Does querying a missing persisted id and publishing an empty payload make deletion safe without
   activating an arbitrary imported row?
3. Do both UI write paths and all parameterless startup/settings call sites now preserve the exact
   intended profile?
4. Please independently verify that the final physical behavior is not an oldest-row false positive.
...
DeepSeek Flash should review and independently test exact remote HEAD, post an APPROVE or
REQUEST-CHANGES verdict with P1/P2 findings, and stop before merge. If approved, return the ball to
Sol/co-creator; do not merge.
...
# BUILD SUCCESSFUL; JVM 354/354, 0 failures/errors/skips
...
# BUILD SUCCESSFUL; 5/5 instrumentation tests
